...
\begin{figure}[h!]
  \begin{center}
    \includegraphics[width=0.70\columnwidth]{figures/sub_circuits3}
...
As indicated in the circuit topology of Figure \ref{fig:baseline_dynamics}, neural populations were driven by a persistent 1 Hz input rate.

We show the steady state rate as a function of the number of baseline connections in the top plots of each neural model in Figure \ref{fig:baseline_dynamics}.
In the bottom plots we display the firing rate dynamics of the model for different number of connections.


\begin{figure}[h!]
  \begin{center}
    \includegraphics[width=1.00\columnwidth]{figures/experiment_001}
...
For all synaptic efficacy values, the firing rate increases smoothly until achieving the steady state after approximately 200 ms.

The AdEx population has a different temporal behavior.
An immediate transient peak of activity on initial stimulation is then driven down by adaptation, achieving a steady state after approximately 600 ms.
...
To implement a binding in a Compartment Circuit, we needed to control, at different moments, the total input rate of the Working Memory population, that was determined by the summed firing rate of the Sub assemblies.
Since steady state rate values were limited in the AdEx model with high synaptic efficacy, it was not possible, in that case, to flexibly operate the circuit with non simultaneous input events.

Because we wanted to explore the behavior of the Compartment circuit for all cases of input events, we decided to restrict all remaining simulations to a 10\% synaptic efficacy.
We also fixed the number of baseline connections to 115 and 1646, for LIF and AdEx populations respectively, since these values best approximated the desired 1Hz steady state firing rate.
...
In the second experiment we explored how the timing of input and control events would affect the rate of activity across a chain of neural populations.
We employed the sub-circuit topology presented in Figure /ref{fig:experiment_2}, in which a fixed input rate drove the activity of the sub-circuit during a given time window, mediated by a control fixed rate acting during a different time window.


\begin{figure}[h!]
  \begin{center}
    \includegraphics[width=0.70\columnwidth]{figures/experiment2_ctl_plots}
...
This corresponds to a Main Assembly (MA)  that drives up the activity successively of a Gate Assembly (G) and Sub Assembly (SA), while G activity is modulated by the control signal. 
Inhibition of the activity in G from the control rate, isolates SA activity from the input stimulation.
...
emph{As discussed earlier, beyond a certain number of excitatory connections there is the possibility of runaway activity in an excitatory loop.
...
Another important implication of the closed loop is that we would expect the steady state activity of the populations to be higher than in their emph{uncoupled states}, which also gives a lower bound to the activation threshold that we can select for WM, since otherwise WM would enter a state of perpetual self activation after being activated the first time. {need to briefly remind that working memory needs a threshold}
...
\begin{figure}[h!]
  \begin{center}
    \includegraphics[width=0.70\columnwidth]{figures/experiment_3}
...
\begin{figure}[h!]
  \begin{center}
    \includegraphics[width=0.70\columnwidth]{figures/experiment_4}
...
\begin{figure}[h!]
  \begin{center}
    \includegraphics[width=0.70\columnwidth]{figures/compartment_circuit_dynamics}
...
First we considered the simple case of increasing size right branching trees that relate to the a recent analysis of intracortical (ECoG) recordings of a phrase reading task\cite{Nelson_2017}.
...
We show the time courses of the simulation 

.~In our simulation, shown in Figure 12.A, we can see four main temporal segments of neural activity.
An initial increase of activity dependent on word presentation that reflects MA populations, a transient activation peak during WM activation, 

\emph{explain in more detail}

a decrease of activity paced by the duration of input activity driving MA populations and a final drop of activity when WM delay activity stops and the inner closed loop of populations in the memory sub-circuit go back to its initial baseline state.
We also show, in Figure 11.B and 11.C., the results from Nelson et al., reflecting the mean high gamma power time series of the ECoG recordings. \emph{truly explain what is being shown}
...
\begin{figure}[h!]
  \begin{center}
    \includegraphics[width=1.00\columnwidth]{figures/ecog_comparison2}
...
In this experiment each stimuli
us presented to a subject in a trial consisted of a list of phrases with the same number of words (constituents), such that in total 12 words would be presented.
...
These stimuli were simulated by repeating and adding the time series of~each of the right branching trees in a condition.
...
We take this as a proxy for BOLD activity.
...
The max of the HRF response is delayed

It can be seen that the neural time series would predict in all cases a peak onset displaced many seconds with respect to the traditional boxcar event that only represents the duration of the stimuli.
...
\begin{figure}[h!]
  \begin{center}
    \includegraphics[width=1.00\columnwidth]{figures/pnas_comparison01}
